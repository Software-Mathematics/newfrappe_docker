...
docker build -f fix-crlf.Dockerfile -t "$IMAGE" .
docker build -f theme.Dockerfile   -t "$IMAGE" .
...
for i in $(seq 1 60); do
  "${COMPOSE[@]}" exec -T backend bench --site frontend list-apps >/dev/null 2>&1 && break
  sleep 5
